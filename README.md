...
starting with **Phase 01: Foundation**.

Phase 1 is a real, verified foundation, not a mockup: authentication,
a durable database with a versioned schema, strict types, health checks
that only report what they verified, and an honest command-center UI.
Future systems (Repository Lab, Agency Director, Website Factory, outreach,
payments, …) are deliberately **not** built yet; the architecture reserves
clean seams for them (see `docs/architecture.md`).
...
- Studio-themed landing page and a polished, honest command center
- Protected studio shell: sidebar, topbar, command palette (⌘K), mobile
  drawer, responsive tables/cards
- Real CRUD for **Leads**, **Websites (projects)**, and **Clients** with
  server-side validation, duplicate prevention, and an **activity log**
  written only by real operations
- Dashboard metrics derived from the database (zeros are zeros)
...
```
src/
  convex/        Backend: schema, entity modules, health checks, lib helpers
  shared/        Domain model imported by both backend and client
...
`?returnTo=...`. All studio routes live under `/dashboard`.
